...
const svgMap = (props) => {
    const {data, filterLote, setfilterLote, setConfirmDialog, confirmDialog, setFilterFn} = props
    return (
        <Fragment>
              ...
              <title>Mapa HASSPARADISE</title>
                {data.map((item) => (
                  <Link onClick={() => {
                    setConfirmDialog({
                      isOpen: true,
                      title: `Estas seguro que deseas buscar por el lote ${item.id}`,
                    ...
                    <path key={item.id} transform={item.transform} fill={item.fill} id={item.id} d={item.d} stroke="#000000"/>
                  </Link>
                ))}
              <text
                fill="#000000"
